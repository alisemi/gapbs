...
pvector<ScoreT> PageRankPullGS(const Graph &g, const std::string filename, const std::string events, const  int max_iters,
  ...
  string inputFileName = std::filesystem::path(filename).replace_extension("").filename();
  std::string result_filename = events;
  replace(result_filename.begin(), result_filename.end(), ',', '-');
  result_filename = "result_PR_" + inputFileName + "_" + result_filename;
...
int main(int argc, char* argv[]) {
  ...
  auto PRBound = [&cli] (const Graph &g) {
    return PageRankPullGS(g, cli.filename(),  cli.events(), cli.max_iters(), cli.tolerance());
  };
